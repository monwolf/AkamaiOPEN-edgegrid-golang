...
func TestAppSec_GetConfigurationVersion(t *testing.T) {

	result := GetConfigurationVersionResponse{}

	respData := compactJSON(loadFixtureBytes("testdata/TestConfigurationVersion/ConfigurationVersionDetail.json"))
	err := json.Unmarshal([]byte(respData), &result)
	require.NoError(t, err)

	tests := map[string]struct {
		params           GetConfigurationVersionRequest
		responseStatus   int
		responseBody     string
		expectedPath     string
		expectedResponse *GetConfigurationVersionResponse
		withError        error
		headers          http.Header
	}{
		"200 OK": {
			...
			responseStatus:   http.StatusOK,
			responseBody:     string(respData),
			expectedPath:     "/appsec/v1/configs/43253/versions/2",
			expectedResponse: &result,
		},
		"500 internal server error": {
			params: GetConfigurationVersionRequest{
			...
			responseBody: `
{
    "type": "internal_error",
    "title": "Internal Server Error",
    "detail": "Error fetching configuration version",
    "status": 500
}`,
			expectedPath: "/appsec/v1/configs/43253/versions/2",
			withError: &Error{
